...
# Source: https://raw.githubusercontent.com/imjp94/gd-plug/b762f285a9ac10902a3f613edeee9633a08d8c2a/addons/gd-plug/plug.gd
@tool
extends SceneTree
...
func show_syntax():
	...
	var actions = {
		...
		"status":
		"Check the status of plugins(installed, added or removed), execute this command whenever in doubts",
	...
	var options = {
		"production":
		"Install only plugins not marked as dev, or uninstall already installed dev plugins",
		"test": "Testing mode, no files will actually be installed/uninstalled",
		"force":
		"Force gd-plug to overwrite destination files when running install command. *WARNING: Check README for more details*",
		"keep-import-file": 'Keep ".import" files generated by plugin, when run uninstall command',
		"keep-import-resource-file":
		'Keep files located in ".import" that generated by plugin, when run uninstall command',
...
func show_config_syntax():
	...
	var arguments = {
		"include":
		'Array of strings that define what files or directory to include. Only "addons/" will be included if omitted',
		...
		"dev":
		"Boolean to mark the plugin as dev or not, plugin marked as dev will not be installed when production command given",
		"on-updated":
		"Post update hook, a function name declared in plug.gd that will be called whenever the plugin installed/updated"
	}
	logger.indent()
	logger.table_start()
		...
		if plugged_dir.current_is_dir():
			if not (file in _installed_plugins):
				...
				threadpool.enqueue_task(
					directory_delete_recursively.bind(plugged_dir.get_current_dir() + "/" + file)
				)
					...
					if test:
						...
						if source.get_file() == ".git":
							...
							if exit == OK:
								...
								logger.debug(
									(
										"Failed to remove empty directory: %s"
										% sub_dir.get_current_dir()
									)
								)
						else:
...
class _GitExecutable:
	...
	func get_commit_comparison(branch_a, branch_b):
		var output = []
		var exit = _execute(
			"git rev-list --count --left-right %s...%s" % [branch_a, branch_b], output
		)
