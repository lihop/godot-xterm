...
func _set_terminal_font() -> void:
	...
	if editor_settings.has_setting("interface/editor/code_font_size"):
		editor_font_size = editor_settings.get_setting("interface/editor/code_font_size")
	...
	if editor_font and editor_font is Font:
		...
		# Apply editor font size if available
		if editor_font_size and editor_font_size is int:
			add_theme_font_size_override("normal_font_size", editor_font_size)
			add_theme_font_size_override("bold_font_size", editor_font_size)
			add_theme_font_size_override("italics_font_size", editor_font_size)
			add_theme_font_size_override("bold_italics_font_size", editor_font_size)
		return
	...
	if ResourceLoader.exists(font_path):
		...
		if default_font and default_font is Font:
			...
			# Use editor font size even with fallback font if available
			if editor_font_size and editor_font_size is int:
				add_theme_font_size_override("normal_font_size", editor_font_size)
				add_theme_font_size_override("bold_font_size", editor_font_size)
				add_theme_font_size_override("italics_font_size", editor_font_size)
